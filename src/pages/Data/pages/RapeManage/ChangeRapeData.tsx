...
import { getCategory, updateDataCategory } from '../../../../api/admin';
...
const Menu_left = (props: IProps) => {
  const { handleCategory, handleIsDataOrImages, location, category } = props;
...
export const ChangeRapeData = (props: IProps) => {
  ...
  console.log("location123444:", location);
  console.log('itemObject :>> ', itemObject);
  // window.localStorage.setItem('YearKey', JSON.stringify(location.query.key))
  ...
  const onChangeItem = () => {
    ...
    if (flag) {
      updateDataCategory({ condition }).then(res => {
        if (res.code === 0) {
    ...
    <div className={$style['rightWrapper']}>
      ...
      <div>
        ...
        <div className={$style['buttons']} >
          <Button type="primary" onClick={onChangeItem}>修改</Button>
          {/* <Modal
            title="提示"
            visible={visible}
